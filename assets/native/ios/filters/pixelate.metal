...
fragment vv_fmain_out fmain(constant VVUniforms& _29 [[buffer(0)]], texture2d<float> uTexture [[texture(0)]], sampler uTextureSmplr [[sampler(0)]], float4 gl_FragCoord [[position]])
{
    ...
    float px = 1.0 + (40.0 * _29.uIntensity);
    float2 dims = _29.uResolution / float2(px);
    float2 uvPixelated = floor(uv * dims) / dims;
    uvPixelated += (float2(0.5) / dims);
    float3 finalColor = uTexture.sample(uTextureSmplr, uvPixelated).xyz;
