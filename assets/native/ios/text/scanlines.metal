...
fragment vv_fmain_out fmain(constant VVUniforms& _31 [[buffer(0)]], float4 gl_FragCoord [[position]])
{
    ...
    float2 uv = frag / res;
    float n = mix(120.0, 300.0, fast::clamp(_31.uThickness, 0.0, 1.0));
    float t = _31.uTime * _31.uSpeed;
    float lines = step(0.5, fract((uv.y + (0.00999999977648258209228515625 * sin(t))) * n));
    float mask = mix(0.60000002384185791015625, 1.0, lines);
    float3 base = mix(_31.uColorA, _31.uColorB, float3(uv.x));
    float3 col = (base * mix(1.0, 1.25, _31.uIntensity)) * mask;
