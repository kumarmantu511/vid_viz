...
{
    float3 p = abs((fract(c.xxx + float3(0.0, 0.666666686534881591796875, 0.3333333432674407958984375)) * 6.0) - float3(3.0));
    float3 rgb = fast::clamp(p - float3(1.0), float3(0.0), float3(1.0));
    return mix(float3(1.0), rgb, float3(c.y)) * c.z;
}
...
fragment vv_fmain_out fmain(constant VVUniforms& _96 [[buffer(0)]], float4 gl_FragCoord [[position]])
{
    ...
    float stripe = (p.x * (8.0 + (16.0 * _96.uThickness))) + t;
    float hue = fract(stripe);
    float sat = 0.800000011920928955078125 + (0.20000000298023223876953125 * _96.uIntensity);
    float val = 0.85000002384185791015625 + (0.1500000059604644775390625 * sin(t * 2.0));
    float3 param_2 = float3(hue, sat, val);
    float3 col = hsv2rgb(param_2);
