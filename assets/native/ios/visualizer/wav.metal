...
fragment vv_fmain_out fmain(constant VVUniforms& _40 [[buffer(0)]], float4 gl_FragCoord [[position]])
{
    ...
    {
        int param = i;
        float amp = bandValue(param, _40);
        float w = 1.0 - (float(i) / 10.0);
        float freq = float(i + 1) / 8.0;
        freq *= freq;
        float phase = (0.300000011920928955078125 + (0.100000001490116119384765625 * float(i))) * _40.uSpeed;
        osc += ((w * amp) * sin(freq * ((uv.x * 1000.0) + ((_40.uTime * 500.0) * phase))));
    }
    osc /= 8.0;
    float targetY = osc * _40.uIntensity;
    float distY = abs(uv.y - targetY);
    float distInPixels = (distY * _40.uResolution.y) * 0.5;
    float baseThickness = fast::max(1.0, _40.uStroke * 2.0);
    ...
    float totalAlpha = fast::clamp(coreAlpha + glowAlpha, 0.0, 1.0);
    if (totalAlpha <= 0.0)
    {
