...
static inline __attribute__((always_inline))
float3 StaticDrops(thread float2& uv, thread const float& t, thread const float& dSize)
{
    uv *= 30.0;
    ...
    float c = (smoothstep(dSize * 0.20000000298023223876953125, 0.0, d) * fract(n.z * 10.0)) * fade;
    return float3(diff * c, c);
}
...
static inline __attribute__((always_inline))
float4 Drops(thread float2& uv, thread const float& t, thread const float& dSize, thread const float& density)
{
    float2 UV = uv;
    uv.y = -uv.y;
    ...
    float2 dropletVec = st - float2(x, y);
    float dd = length(dropletVec);
    float param_4 = id.x;
    float dropletMask = smoothstep(currentDropSize * N(param_4), 0.0, dd);
    ...
    float2 finalNormal = ((dropVec * Drop) + (dropletVec * droplets)) * borderMask;
    return float4(finalNormal, m * borderMask, trail * borderMask);
}

static inline __attribute__((always_inline))
float4 Rain(thread const float2& uv, thread const float& t, thread const float& dSize, thread const float& density)
{
    float2 param = uv;
    float param_1 = t;
    float param_2 = dSize;
    float3 _475 = StaticDrops(param, param_1, param_2);
    float3 s = _475;
    float2 param_3 = uv;
    float param_4 = t;
    float param_5 = dSize;
    float param_6 = density;
    float4 _486 = Drops(param_3, param_4, param_5, param_6);
    float4 r1 = _486;
    float2 param_7 = uv * 1.7999999523162841796875;
    float param_8 = t;
    float param_9 = dSize;
    float param_10 = density;
    float4 _498 = Drops(param_7, param_8, param_9, param_10);
    float4 r2 = _498;
    float2 normal = (s.xy + r1.xy) + r2.xy;
    float c = (s.z + r1.z) + r2.z;
    c = smoothstep(0.300000011920928955078125, 1.0, c);
    return float4(normal, c, fast::max(r1.w, r2.w));
}

fragment vv_fmain_out fmain(constant VVUniforms& _540 [[buffer(0)]], texture2d<float> uTexture [[texture(0)]], sampler uTextureSmplr [[sampler(0)]], float4 gl_FragCoord [[position]])
{
    vv_fmain_out out = {};
    float2 fragCoord = FlutterFragCoord(gl_FragCoord).xy;
    float2 uv = (fragCoord - (_540.uResolution * 0.5)) / float2(_540.uResolution.y);
    float2 UV = fragCoord / _540.uResolution;
    ...
    float T = _540.uTime * fast::clamp(_540.uSpeed, 0.20000000298023223876953125, 3.0);
    float t = T * 0.20000000298023223876953125;
    float dSize = fast::clamp(_540.uDropSize, 0.5, 2.0);
    float density = fast::clamp(_540.uDensity, 0.100000001490116119384765625, 1.0);
    UV = ((UV - float2(0.5)) * 0.980000019073486328125) + float2(0.5);
    float2 param = uv;
    float param_1 = t;
    float param_2 = dSize;
    float param_3 = density;
    float4 data = Rain(param, param_1, param_2, param_3);
    float2 n = data.xy;
    float mask = data.z;
    float trail = data.w;
    float3 foggyBg = mix(baseColor.xyz, float3(0.949999988079071044921875), float3(0.20000000298023223876953125));
    float3 clearDrop = uTexture.sample(uTextureSmplr, (UV + (n * 2.0))).xyz;
    float3 finalRGB = mix(foggyBg, clearDrop, float3(fast::clamp(mask + (trail * 0.5), 0.0, 1.0)));
    finalRGB += float3(trail * 0.100000001490116119384765625);
    finalRGB = mix(baseColor.xyz, finalRGB, float3(fast::clamp(_540.uIntensity, 0.0, 1.0)));
